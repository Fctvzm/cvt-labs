...
classes = ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'K', 'L', 'M', 'N', 'O', 'P', 'Q', 'R', 'S', 'T', 'V', 'X', 'Y' 'Z']
...
session = tf.Session()
x = tf.placeholder(tf.float32, shape=[None, img_sizeH, img_sizeW], name='x')
...
def create_convolutional_layer(input, filter_size, n_filters):  
    weights = create_weights(shape = [filter_size, filter_size, n_filters])
    biases = create_biases(n_filters)

    layer = tf.nn.conv2d(input = input, filter = weights, strides = [1, 1, 1], padding = 'SAME')
    layer += biases

    layer = tf.nn.max_pool(value = layer, ksize = [1, 2, 2], strides = [1, 2, 2], padding = 'SAME')
...
layer_conv1 = create_convolutional_layer(x, filter_size_conv1, num_filters_conv1)
